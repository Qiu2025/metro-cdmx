...
def crear_grafo():
    ...
    def P(n):
        if n in coords:
            return coords[n]
        print("⚠ No encontrada:", n)
        return (0,0)

    # -------------------------------
    # NODOS (IGUAL QUE TU CÓDIGO)
    # -------------------------------
    ...
    for (a,b),d in DISTANCIAS_REALES.items():
        G.add_edge(a,b,weight=d/VELOCIDAD_METRO)
...
class AppMetro:
    def __init__(self,root):
        ...
        self.row.pack(fill="x", padx=12, pady=8)

        # TÍTULO
        ctk.CTkLabel(self.row, text="Metro CDMX",
                    font=("Helvetica", 20, "bold"),
                    text_color="#222").pack(side="left", padx=(5,20))
        ...
        self.cb_origen = ctk.CTkComboBox(self.row, values=self.nombres_estaciones,
                                        width=150, height=32,
                                        corner_radius=10)
        self.cb_origen.pack(side="left", padx=(5,15))
        ...
        self.cb_destino = ctk.CTkComboBox(self.row, values=self.nombres_estaciones,
                                        width=150, height=32,
                                        corner_radius=10)
        self.cb_destino.pack(side="left", padx=(5,15))
        ...
        ctk.CTkCheckBox(self.row, text="Escaleras",
                        variable=self.var_escal).pack(side="left", padx=5)
        ctk.CTkCheckBox(self.row, text="Ascensor",
                        variable=self.var_ascen).pack(side="left", padx=5)
        ...
        self.btn_buscar.pack(side="left", padx=(20,10))

        # BOTÓN VOLVER (oculto)
        self.btn_retroceder = ctk.CTkButton(self.row, text="Volver",
                                            fg_color="#E0E0E0", text_color="#222",
                                            hover_color="#C8C8C8",
                                            corner_radius=10,
                                            width=80,
                                            command=self.retroceder)

        # Label de info visual
        self.lbl = ctk.CTkLabel(self.row, text="", text_color="#333")
        self.lbl.pack(side="left", padx=10)



        # -------- ZONA CENTRAL (MAPA IZQUIERDA + PANEL INFO DERECHA) --------
        self.main_area = tk.Frame(root)
        self.main_area.pack(fill="both", expand=True)

        # PANEL MAPA (IZQUIERDA)
        self.panel_mapa = tk.Frame(self.main_area)
        self.panel_mapa.pack(side="left", fill="both", expand=True)

        self.canvas = tk.Canvas(self.panel_mapa, bg="white")
        self.canvas.pack(fill="both", expand=True)

        # PANEL INFO (DERECHA)
        self.panel_info = ctk.CTkFrame(self.main_area, width=280, fg_color="white",
                                    border_width=1, border_color="#DDDDDD",
                                    corner_radius=0)
        self.panel_info.pack(side="right", fill="y")

        # Título del panel info
        self.lbl_info_title = ctk.CTkLabel(self.panel_info, text="Detalles de la ruta",
                                        font=("Helvetica", 18, "bold"),
                                        text_color="#222")
        self.lbl_info_title.pack(pady=10)

        # Caja donde se pondrá la info
        self.text_info = tk.Text(self.panel_info, wrap="word", font=("Helvetica", 12))
    ...
    def on_node_click(self,node):
        ...
        self.lbl.configure(text=f"Seleccionada: {nombre}")
        self.redibujar()
    ...
    def calcular(self):
        ...
        self.ruta=ruta
        self.lbl.configure(text=f"Tiempo: {math.ceil(tiempo/60)} min\nEstaciones: {len(ruta)}")
        # -------- PANEL DETALLE DE RUTA --------
        self.text_info.delete("1.0", tk.END)

        self.text_info.insert(tk.END, f"Tiempo estimado: {math.ceil(tiempo/60)} min\n")
        self.text_info.insert(tk.END, f"Total estaciones: {len(ruta)}\n\n")

        self.text_info.insert(tk.END, "Recorrido:\n")
        self.text_info.insert(tk.END, "-----------------------------\n")

        for nodo in ruta:
            nombre = self.graph.nodes[nodo]["nombre"]
            linea = self.graph.nodes[nodo]["linea"]
            self.text_info.insert(tk.END, f"• {nombre} ({linea})\n")
    ...
    def dibujar_ruta(self):
        ...
        X,Y=self.convertir(x,y)
        self.canvas.create_oval(X-10,Y-10,X+10,Y+10, fill="#39FF14", outline="black", width=2)

        # fin
        b=self.ruta[-1]
        x,y=self.graph.nodes[b]["pos"]
        X,Y=self.convertir(x,y)
        self.canvas.create_oval(X-10,Y-10,X+10,Y+10, fill="red", outline="black", width=2)
